...
def get_essay_details():
    ...
    essay_type=input("What type of essay do you want to write? (e.g., argumentative, descriptive) ").strip()
...
def generate_essay_content(details):
    try:
        temp=float(input("Enter temeprature (0.1 structured, 0.7 creative): "))
        if not (0.0<=temp<=1.0): raise ValueError:
    except ValueError:
        print("Invalid temperature, using default 0.3")
        temp=0.3
    intro_p=f"Write an intro for an {details['essay_type']} essay about {details['topic']} on the topic of {details['length']}."
